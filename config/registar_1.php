...
$sql = "Select iduser, username from pweb.user where username = '{$user}' and email = '{$email}'";
...
if ($row > 0) {
    ...
    $sql = "INSERT INTO pweb.user (username, password, email) VALUES ('{$user}', md5('{$password}'), '{$email}')";
    $result = mysqli_query($conectar, $sql);
